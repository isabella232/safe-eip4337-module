...
describe('RelayModuleFixedReward', async () => {
    ...
    describe('relayTransactionOptimistic', async () => {
        it('should require enabled module', async () => {
            ...
            const execTransactionData = executor.interface.encodeFunctionData('execTransaction', [
                mock.address,
                0,
                '0xbaddad42',
                0,
            ...
            await expect(module.relayTransactionOptimistic(executor.address, execTransactionData, user2.address)).to.be.revertedWith(
                'TestExecutor: Not authorized',
            )
        })
        ...
        it('should check if payment happened', async () => {
            ...
            await expect(module.relayTransactionOptimistic(mock.address, execTransactionData, user2.address)).to.be.revertedWith(
                'RewardPaymentMissing()',
            )
        })
        ...
        it('should only allow defined method', async () => {
            ...
            await expect(await hre.ethers.provider.getBalance(executor.address)).to.be.equals(parseEther('0.005'))
            await expect(module.relayTransactionOptimistic(executor.address, execData, user2.address)).to.be.revertedWith(
                'InvalidRelayData()',
            )
            await expect(await hre.ethers.provider.getBalance(executor.address)).to.be.equals(parseEther('0.005'))
        })
        ...
        it('should not revert if successful', async () => {
            ...
            await expect(await hre.ethers.provider.getBalance(executor.address)).to.be.equals(0)

            // Check that target was called exactly 1 time with expected data
            await expect(await mock.callStatic.invocationCount()).to.be.equals(1)
            await expect(await mock.callStatic.invocationCountForMethod('0xbaddad42')).to.be.equals(1)
        })
