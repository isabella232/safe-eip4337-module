...
describe('EnableModulesLib', async () => {
    ...
    describe('enableModules', async () => {
        it('should revert if used with call', async () => {
            ...
            await expect(executor.exec(lib.address, 0, data, 0)).to.be.reverted
            await expect(await executor.moduleCount()).to.be.equal(0)
        })
        ...
        it('should enable module', async () => {
            ...
            await expect(await executor.moduleCount()).to.be.equal(1)
            await expect(await executor.modules(mock.address)).to.be.equal(true)
        })
        ...
        it('should enable multiple module', async () => {
            ...
            await executor.exec(lib.address, 0, data, 1)
            await expect(await executor.moduleCount()).to.be.equal(2)
            await expect(await executor.modules(mock.address)).to.be.equal(true)
            await expect(await executor.modules(user2.address)).to.be.equal(true)
        })
