...
describe('SafeEIP4337Diatomic', async () => {
  ...
  describe('validateUserOp', () => {
    ...
    it("should revert if the operation nonce doesn't match current safe nonce", async () => {
      ...
      await expect(eip4337Safe.validateUserOp(userOp, `0x${'0'.repeat(64)}`, 0)).to.be.revertedWith('Nonce mismatch')
    })
    ...
    it("should revert if msg.sender doesn't match signed entry point", async () => {
      ...
      await expect(user2Safe.validateUserOp(userOp, `0x${'0'.repeat(64)}`, 0)).to.be.revertedWith('Invalid signature')
    })
    ...
    it('should revert if the entrypoint asks for a pre-fund larger than defined in the UserOperation', async () => {
      ...
      await expect(eip4337Safe.validateUserOp(userOp, `0x${'0'.repeat(64)}`, requiredPrefund)).to.be.revertedWith(
        'Prefund quoted by the entrypoint is larger than one in the operation',
      )
    })
    ...
    it('should revert if called not through the safe', async () => {
      ...
      ).to.be.revertedWith('Can be only called through the Safe')
    })
  ...
  describe('execTransaction', () => {
    ...
    it('should revert if called not by the entry point from validateUserOp', async () => {
      ...
      await expect(user2.sendTransaction({ to: eip4337Safe.address, data: userOp.callData })).to.be.revertedWith(
        'Execution error: transaction isnt ready to execute',
      )
    })
